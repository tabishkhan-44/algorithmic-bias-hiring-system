"""
=============================================================================
Attention-Based Hiring Classifier — built from scratch in NumPy
=============================================================================
...
access to install PyTorch or TensorFlow, so the attention mechanism —
Q/K/V projections, scaled dot-product softmax attention, and the full
backward pass — is implemented and hand-verified below using only NumPy.
This is the same core mathematics used inside a Transformer's
self-attention block (Vaswani et al., "Attention Is All You Need",
NeurIPS 2017), applied here to tabular HIRING features instead of words.

WHY ATTENTION FOR A BIAS PROJECT SPECIFICALLY?
Beyond being "one more algorithm" for the comparison table, attention
gives us something the other five models can't: an interpretable
per-feature attention weight. We can check, directly, whether the model
is "paying attention" to protected attributes (Gender, Race_Ethnicity,
Religion, Continent) as much as it pays attention to legitimate merit
features (Technical_Score, Experience_Years) — turning the model's own
internal weighting into a bias-detection tool.

Run:  python3 06_attention_model.py
=============================================================================
"""
import numpy as np
import pandas as pd
...
class AttentionHiringModel:
    """Single-head feature-wise self-attention classifier: every input
    feature is treated as a 'token', attends to every other feature, and
    the resulting context vector feeds a small classification head."""
    ...
    def mean_attention_per_feature(self, X, batch_size=1024, sample=6000):
        if X.shape[0] > sample:
...
def main():
    print("=" * 70)
    print("  ATTENTION-BASED HIRING CLASSIFIER (NumPy, from scratch)")
    print("=" * 70)
    ...
    model = AttentionHiringModel(n_features=X.shape[1], d_model=8, hidden=32, lr=2e-3, seed=42)
    print("\n[1] Training feature-wise self-attention network...")
    ...
    print("\n[3] Fairness check (Attention Network vs 4 protected attributes):")
    ...
    print("\n[4] Mean attention received per feature (top 10):")
    attn = model.mean_attention_per_feature(X)
    attn_df = pd.DataFrame({'feature': feature_names, 'mean_attention': attn})
    attn_df = attn_df.sort_values('mean_attention', ascending=False)
    print(attn_df.head(10).to_string(index=False))

    protected_features = {f'{c}_enc' for c in cat_cols}
    protected_rank = attn_df[attn_df['feature'].isin(protected_features)]
    print(f"\n    -> Protected attributes occupy ranks "
          f"{[list(attn_df['feature']).index(f)+1 for f in protected_rank['feature']]} "
          f"out of {len(feature_names)} features by attention received.")

    attn_df.to_csv('../data/attention_feature_weights_v2.csv', index=False)
